...
export function UpdateRole({ id }: UpdateRoleProps) {
  ...
  const fetchRole = async () => {
    try {
      const res = await apiService.get(`/api/admin/role/${id}`);
      const role = res?.data;

      const rolePermissionIds =
        role?.permissions?.map((p: z.infer<typeof permission>) => p._id) || [];

      const selectedPermissions = permissions.filter((p) =>
        rolePermissionIds.includes(p._id)
      );
  ...
  const fetchPermissions = async () => {
    try {
      const res = await apiService.get("/api/admin/permission");
  ...
  async function onSubmit(values: z.infer<typeof roleFormSchema>) {
    try {
      ...
      const payload = {
        name: values.name,
        permissions: values.permissions.map((p) => p._id),
      };

      await apiService.put(`/api/admin/role/${id}`, payload);
  ...
  return (
    <div className="px-6 py-8 bg-white rounded-2xl">
      <h2 className="text-md font-semibold mb-6 text-gray-800">Update Role</h2>
      <Form {...form}>
